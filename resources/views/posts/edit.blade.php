...
<div class="container">
    <h1>修改公告</h1>
    {{ Form::model($post,['route' => ['posts.update',$post->id], 'method' => 'PATCH','id'=>'setup', 'files' => true,'onsubmit'=>'return false;']) }}
    <div class="card my-4">
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                @if($title_image)
                    ...
                    ?>
                <a href="{{ url('posts/'.$file.'/fileDel') }}" class="btn btn-danger btn-sm" id="fileDel" onclick="bbconfirm_Link('fileDel','確定刪標題圖片？')"><i class="fas fa-times-circle"></i> 刪</a>
                @endif
            ...
            <div class="form-group">
                ...
                @if(!empty($files))
                    @foreach($files as $k=>$v)
                        ...
                        $file = str_replace('/','&',$file);
                        ?>
                        <a href="{{ url('posts/'.$file.'/fileDel') }}" class="btn btn-danger btn-sm" id="fileDel{{ $k }}" onclick="bbconfirm_Link('fileDel{{ $k }}','確定刪附件？')"><i class="fas fa-times-circle"></i> {{ $v }}</a>
                    @endforeach
                @endif
